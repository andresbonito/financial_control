...
import {
  formatCurrency, formatDate, todayString,
} from '../lib/utils'
import {
...
export function Transactions() {
  ...
  const totalIncome = filtered.filter((t) => isIncome(t.type)).reduce((a, t) => a + t.amount, 0)
  const totalExpense = filtered.filter((t) => !isIncome(t.type)).reduce((a, t) => a + t.amount, 0)
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="p-4">
          ...
          <p className={`text-xl font-bold ${totalIncome - totalExpense >= 0 ? 'text-violet-400' : 'text-red-400'}`}>
                ...
                <div key={tx.id} className="flex items-center gap-4 p-4 hover:bg-slate-700/30 transition-colors">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${income ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
                    {income
                      ...
                      : <ArrowDownRight className="w-4.5 h-4.5 text-red-400" />
                    }
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs px-1.5 py-0.5 bg-slate-700 rounded text-violet-400">{getTransactionLabel(tx.type)}</span>
                      {tx.payment_method && (
                        <span className="text-xs px-1.5 py-0.5 bg-slate-700 rounded text-amber-400">{getPaymentMethodLabel(tx.payment_method)}</span>
                      )}
                    </div>
                  </div>
                  <span className={`text-base font-bold ${income ? 'text-green-400' : 'text-red-400'}`}>
                    {income ? '+' : '-'}{formatCurrency(tx.amount)}
                  </span>
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(tx)} className="p-1.5 hover:bg-slate-700 rounded-lg text-slate-400 hover:text-slate-100 transition-colors">
                      <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteTransaction(tx.id)} className="p-1.5 hover:bg-red-500/20 rounded-lg text-slate-400 hover:text-red-400 transition-colors">
